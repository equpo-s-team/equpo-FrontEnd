            ...
            <div className="max-w-[1160px] mx-auto grid md:grid-cols-2 gap-20 items-center">
                ...
                <div className="reveal" ref={ref}>
                    <div className="relative">
                        ...
                        {/* World widget */}
                        <div
                            className="absolute -bottom-6 -right-6 bg-white border border-border rounded-2xl px-5 py-4 shadow-[0_12px_40px_rgba(0,0,0,.10)] flex items-center gap-3 min-w-[180px]">
                            <div className="world-mini-orb w-11 h-11 flex-shrink-0 animate-float-orb-fast"/>
                            <div className="font-body text-[.75rem]">
                                <strong className="block font-semibold text-dark text-[.82rem]">Mundo
                                    prosperando</strong>
                                <span className="text-muted">Equipo en racha <TrendingUp size={14}
                                                                                         className="inline text-green ml-1"/></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {/* ── TEXT ── */}
                <div>
                    <SectionLabel color="text-cyan" barColor="bg-cyan" className="reveal reveal-d1">Qué es
                        equpo</SectionLabel>
    ...
    return (
        <div>
            <div className="flex items-center gap-1.5 mb-2">
                ...
                <span
                    className="font-body text-[.7rem] font-semibold uppercase tracking-[.08em] text-muted">{title}</span>
            </div>
            {tasks.map(({tag, tagClass, text}) => (
                <div key={text}
                     className="mock-task bg-offwhite border border-border rounded-[10px] px-[.65rem] py-[.55rem] text-[.72rem] text-dark mb-[.45rem]">
