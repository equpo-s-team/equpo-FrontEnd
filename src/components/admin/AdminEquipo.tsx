...
const ROLES: Role[] = [
  'Frontend Dev','Backend Dev','UX / Design','QA Engineer',
  'DevOps','Full Stack','Product Manager','Mobile Dev','Data Analyst',
]
...
const INITIAL_MEMBERS: Member[] = [
  { id:'at', initials:'AT', name:'Andrea Torres',  role:'Frontend Dev',    email:'andrea@orbit.io',  status:'active',  completionPct:83, tasksCompleted:24, tasksTotal:29, avatarGradient:AVATAR_GRADIENTS[0].gradient, avatarGlow:AVATAR_GRADIENTS[0].glow },
  { id:'jr', initials:'JR', name:'Jorge Ramírez',  role:'Backend Dev',     email:'jorge@orbit.io',   status:'active',  completionPct:72, tasksCompleted:18, tasksTotal:22, avatarGradient:AVATAR_GRADIENTS[1].gradient, avatarGlow:AVATAR_GRADIENTS[1].glow },
  { id:'ml', initials:'ML', name:'María López',    role:'UX / Design',     email:'maria@orbit.io',   status:'active',  completionPct:91, tasksCompleted:21, tasksTotal:23, avatarGradient:AVATAR_GRADIENTS[2].gradient, avatarGlow:AVATAR_GRADIENTS[2].glow },
  { id:'cs', initials:'CS', name:'Carlos Soto',    role:'QA Engineer',     email:'carlos@orbit.io',  status:'away',    completionPct:92, tasksCompleted:25, tasksTotal:27, avatarGradient:AVATAR_GRADIENTS[3].gradient, avatarGlow:AVATAR_GRADIENTS[3].glow },
  { id:'lv', initials:'LV', name:'Laura Vargas',   role:'DevOps',          email:'laura@orbit.io',   status:'active',  completionPct:68, tasksCompleted:16, tasksTotal:20, avatarGradient:AVATAR_GRADIENTS[4].gradient, avatarGlow:AVATAR_GRADIENTS[4].glow },
  { id:'dm', initials:'DM', name:'Diego Morales',  role:'Full Stack',      email:'diego@orbit.io',   status:'away',    completionPct:73, tasksCompleted:11, tasksTotal:15, avatarGradient:AVATAR_GRADIENTS[5].gradient, avatarGlow:AVATAR_GRADIENTS[5].glow },
  { id:'rp', initials:'RP', name:'Renata Paredes', role:'Product Manager', email:'renata@orbit.io',  status:'active',  completionPct:86, tasksCompleted:19, tasksTotal:22, avatarGradient:AVATAR_GRADIENTS[6].gradient, avatarGlow:AVATAR_GRADIENTS[6].glow },
  { id:'kn', initials:'KN', name:'Kevin Núñez',   role:'Mobile Dev',      email:'kevin@orbit.io',   status:'offline', completionPct:76, tasksCompleted:13, tasksTotal:17, avatarGradient:AVATAR_GRADIENTS[7].gradient, avatarGlow:AVATAR_GRADIENTS[7].glow },
  { id:'so', initials:'SO', name:'Sofía Ortega',  role:'Data Analyst',    email:'sofia@orbit.io',   status:'active',  completionPct:66, tasksCompleted:8,  tasksTotal:12, avatarGradient:AVATAR_GRADIENTS[8].gradient, avatarGlow:AVATAR_GRADIENTS[8].glow },
]
    ...
    <div aria-hidden className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
      {[
        ...
        <div key={i} className="absolute rounded-full" style={{
          width:o.w, height:o.h, background:o.bg, filter:'blur(90px)',
          ...('top' in o && typeof o.top === 'number' ? {top:o.top} : 'top' in o ? {top:o.top} : {}),
          ...('bottom' in o && typeof o.bottom === 'number' ? {bottom:o.bottom} : 'bottom' in o ? {bottom:o.bottom} : {}),
          ...('left' in o && typeof o.left === 'number' ? {left:o.left} : 'left' in o ? {left:o.left} : {}),
          ...('right' in o && typeof o.right === 'number' ? {right:o.right} : 'right' in o ? {right:o.right} : {}),
          animation:`floatOrb 9s ease-in-out infinite`, animationDelay:o.delay,
        }}/>
      ))}
    </div>
  )
...
/* ─── Panel wrapper ─── */
function Panel({ children, className = '', glowColor = '' }: { children: React.ReactNode, className?: string, glowColor?: string }) {
  return (
    <div className={`relative bg-white border border-[#EEECEA] rounded-2xl overflow-hidden ${className}`}
...
function NeonBar({ gradient, shadow }: { gradient: string, shadow: string }) {
  return <div className="absolute top-0 left-0 right-0 h-[3px] rounded-t-2xl" style={{ background:gradient, boxShadow:shadow }}/>
}

/* ─── Avatar ─── */
function Avatar({ member, size = 'md' }: { member: Pick<Member,'initials'|'avatarGradient'|'avatarGlow'>, size?: 'sm'|'md'|'lg' }) {
  const sz = size === 'lg' ? 'w-16 h-16 text-base' : size === 'sm' ? 'w-8 h-8 text-[0.6rem]' : 'w-10 h-10 text-[0.72rem]'
  return (
    <div className={`${sz} rounded-full flex items-center justify-center font-semibold text-white flex-shrink-0 tracking-wide`}
...
function StatPill({ value, label, color }: { value: string|number, label: string, color: string }) {
  return (
    <div className="flex flex-col items-center px-3 py-1.5 rounded-xl bg-[#FAFAF8] border border-[#EEECEA]">
      <span className="text-sm font-semibold tracking-tight" style={{ color }}>{value}</span>
...
function TeamInfoPanel({ team, onEdit }: { team: TeamInfo, onEdit: () => void }) {
  ...
  const avgCompletion = Math.round(INITIAL_MEMBERS.reduce((a,m) => a + m.completionPct, 0) / INITIAL_MEMBERS.length)

  return (
    <Panel glowColor="rgba(96,175,255,0.09)">
      <NeonBar gradient="linear-gradient(90deg,#60AFFF,#5961F9)" shadow="0 0 18px rgba(96,175,255,0.7)"/>
          ...
          <div className="relative flex-shrink-0">
            {team.avatarUrl ? (
              ...
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-bold text-white"
                style={{ background:team.avatarGradient, boxShadow:'0 0 20px rgba(96,175,255,0.4)' }}>
                {team.name.charAt(0)}
              </div>
            )}
            <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-[#9CEDC1] border-2 border-white"
              style={{ boxShadow:'0 0 8px rgba(156,237,193,0.9)' }}/>
          </div>

          <div className="flex-1 min-w-0">
            <h2 className="text-lg font-semibold text-[#1A1815] tracking-tight leading-tight truncate">{team.name}</h2>
            <p className="text-xs text-[#908E88] mt-1 line-clamp-2 leading-relaxed">{team.description}</p>
          </div>
...
function PerformanceStrip() {
  const stats = [
    { label:'Completadas', value:61, total:119, color:'#9CEDC1', glow:'rgba(156,237,193,0.8)', pct: Math.round(61/119*100) },
    { label:'En progreso', value:18, total:119, color:'#86F0FD', glow:'rgba(134,240,253,0.8)', pct: Math.round(18/119*100) },
    { label:'En QA',       value: 9, total:119, color:'#FF94AE', glow:'rgba(255,148,174,0.8)', pct: Math.round(9/119*100)  },
    { label:'Por hacer',   value:24, total:119, color:'#9b7fe1', glow:'rgba(155,127,225,0.8)', pct: Math.round(24/119*100) },
    { label:'Vencidas',    value: 7, total:119, color:'#F65A70', glow:'rgba(246,90,112,0.8)', pct: Math.round(7/119*100)  },
  ]

  return (
    <Panel glowColor="rgba(156,237,193,0.08)">
      <NeonBar gradient="linear-gradient(90deg,#9CEDC1,#CEFB7C)" shadow="0 0 14px rgba(156,237,193,0.7)"/>
      <div className="p-5 pt-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[0.85rem] font-semibold text-[#35322E] tracking-tight">Rendimiento general</h3>
        ...
        <div className="flex w-full h-2 rounded-full overflow-hidden gap-0.5 mb-3">
          {stats.map(s => (
            <div key={s.label} className="h-full rounded-full transition-all duration-700"
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1.5">
          {stats.map(s => (
            <div key={s.label} className="flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full" style={{ background:s.color, boxShadow:`0 0 5px ${s.glow}` }}/>
...
}: { member: Member, onEdit: (m: Member) => void, onRemove: (id: string) => void }) {
  const statusCfg = STATUS_CONFIG[member.status]

  return (
    <div className="group relative bg-white border border-[#EEECEA] rounded-2xl p-4 transition-all duration-200
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[0.85rem] font-semibold text-[#1A1815] leading-tight truncate">{member.name}</p>
      ...
      <div className="mt-3.5">
        <div className="flex justify-between mb-1.5 text-[0.64rem] text-[#908E88]">
          ...
          <span className="font-medium text-[#524F4A]">{member.tasksCompleted}/{member.tasksTotal}</span>
        </div>
        <div className="w-full h-1.5 bg-[#EEECEA] rounded-full overflow-hidden">
          <div className="h-full rounded-full transition-all duration-700"
            style={{ width:`${member.completionPct}%`, background:member.avatarGradient, boxShadow:member.avatarGlow }}/>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        <span className="text-[0.7rem] font-semibold" style={{ color:member.avatarGradient.includes('#60AFFF') ? '#5961F9' : '#908E88' }}>
...
function MemberListRow({ member, onEdit, onRemove }: { member: Member, onEdit: (m: Member) => void, onRemove: (id: string) => void }) {
  const statusCfg = STATUS_CONFIG[member.status]
  return (
    <div className="group grid items-center gap-3 py-3 px-4 border-b border-[#F5F4F1] last:border-0
      ...
      <div>
        ...
        <div className="w-full h-1 bg-[#EEECEA] rounded-full overflow-hidden">
          <div className="h-full rounded-full" style={{ width:`${member.completionPct}%`, background:member.avatarGradient }}/>
        </div>
      </div>
...
}: { member: Member | null, onSave: (m: Member) => void, onClose: () => void }) {
  const isNew = !member?.id || member.id === '__new__'
  const [form, setForm] = useState<Member>(member ?? {
    id: `m_${Date.now()}`, initials:'', name:'', role:'Frontend Dev',
    email:'', status:'active', completionPct:0, tasksCompleted:0, tasksTotal:0,
    avatarGradient: AVATAR_GRADIENTS[0].gradient, avatarGlow: AVATAR_GRADIENTS[0].glow,
  })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl"
        ...
        <div className="absolute top-0 left-0 right-0 h-[3px]"
          style={{ background:'linear-gradient(90deg,#60AFFF,#9b7fe1)', boxShadow:'0 0 18px rgba(96,175,255,0.7)' }}/>

        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#EEECEA]">
          <div>
            <h3 className="text-[0.95rem] font-semibold text-[#1A1815]">{isNew ? 'Agregar miembro' : 'Editar miembro'}</h3>
            <p className="text-[0.7rem] text-[#908E88] mt-0.5">{isNew ? 'Completa la información del nuevo integrante' : 'Actualiza los datos del miembro'}</p>
          </div>
          <button onClick={onClose} className="w-7 h-7 rounded-lg bg-[#F5F4F1] flex items-center justify-center text-[#908E88] hover:text-[#35322E] transition-colors">
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Avatar color picker */}
          <div>
            <label className="text-[0.68rem] font-semibold uppercase tracking-wider text-[#B0ADA7] block mb-2.5">Color de avatar</label>
            <div className="flex gap-2 flex-wrap">
              {AVATAR_GRADIENTS.map((ag, i) => (
                <button key={i} onClick={() => { updateField('avatarGradient', ag.gradient); updateField('avatarGlow', ag.glow) }}
                  ...
                  style={{
                    ...
                    outline: form.avatarGradient === ag.gradient ? '2px solid rgba(96,175,255,0.5)' : 'none',
                    outlineOffset: '2px',
                  }}/>
              ))}
              {/* Preview */}
              <div className="ml-auto flex items-center justify-center w-8 h-8 rounded-full text-[0.65rem] font-bold text-white"
          ...
          {/* Name */}
          <div>
            <label className="text-[0.68rem] font-semibold uppercase tracking-wider text-[#B0ADA7] block mb-1.5">Nombre completo</label>
          ...
          {/* Email */}
          <div>
            <label className="text-[0.68rem] font-semibold uppercase tracking-wider text-[#B0ADA7] block mb-1.5">Email</label>
            <input value={form.email} onChange={e => updateField('email', e.target.value)}
              type="email" placeholder="ana@orbit.io"
              className="w-full px-3 py-2.5 rounded-xl border border-[#E4E2DE] text-[0.82rem] text-[#35322E] bg-[#FAFAF8] outline-none
                focus:border-[rgba(96,175,255,0.5)] focus:shadow-[0_0_0_3px_rgba(96,175,255,0.1)] focus:bg-white transition-all"/>
          </div>
          ...
          {/* Status */}
          <div>
            <label className="text-[0.68rem] font-semibold uppercase tracking-wider text-[#B0ADA7] block mb-2">Estado</label>
              ...
              {(['active','away','offline'] as const).map(s => {
                ...
                return (
                  <button key={s} onClick={() => updateField('status', s)}
                    ...
                    <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background:cfg.dot, boxShadow:form.status===s ? cfg.glow : 'none' }}/>
                    {cfg.label}
                  </button>
                )
              })}
...
/* ─── EDIT TEAM MODAL ─── */
function TeamModal({ team, onSave, onClose }: { team: TeamInfo, onSave: (t: TeamInfo) => void, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl"
        ...
        <div className="absolute top-0 left-0 right-0 h-[3px]"
          style={{ background:'linear-gradient(90deg,#9CEDC1,#86F0FD)', boxShadow:'0 0 18px rgba(156,237,193,0.7)' }}/>

        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#EEECEA]">
          <div>
            <h3 className="text-[0.95rem] font-semibold text-[#1A1815]">Editar equipo</h3>
            <p className="text-[0.7rem] text-[#908E88] mt-0.5">Imagen, nombre y descripción del equipo</p>
          </div>
          <button onClick={onClose} className="w-7 h-7 rounded-lg bg-[#F5F4F1] flex items-center justify-center text-[#908E88] hover:text-[#35322E] transition-colors">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Image upload */}
          <div>
            <label className="text-[0.68rem] font-semibold uppercase tracking-wider text-[#B0ADA7] block mb-2.5">Imagen del equipo</label>
            <div className="flex items-center gap-4">
              <div className="relative w-16 h-16 rounded-2xl overflow-hidden flex-shrink-0 cursor-pointer group"
                onClick={() => fileRef.current?.click()}
                style={{ background: form.avatarUrl ? 'transparent' : form.avatarGradient, boxShadow:'0 0 16px rgba(96,175,255,0.25)' }}>
                {form.avatarUrl
                  ? <img src={form.avatarUrl} alt="" className="w-full h-full object-cover"/>
                  : <span className="w-full h-full flex items-center justify-center text-2xl font-bold text-white">{form.name.charAt(0)}</span>
                }
          ...
          {!form.avatarUrl && (
            <div>
              <label className="text-[0.68rem] font-semibold uppercase tracking-wider text-[#B0ADA7] block mb-2">Color de fondo</label>
              <div className="flex gap-2 flex-wrap">
                {AVATAR_GRADIENTS.map((ag, i) => (
                  <button key={i} onClick={() => setForm(f => ({...f, avatarGradient: ag.gradient}))}
          ...
          {/* Name */}
          <div>
            <label className="text-[0.68rem] font-semibold uppercase tracking-wider text-[#B0ADA7] block mb-1.5">Nombre del equipo</label>
          ...
          {/* Description */}
          <div>
            <label className="text-[0.68rem] font-semibold uppercase tracking-wider text-[#B0ADA7] block mb-1.5">Descripción</label>
        ...
        <div className="flex gap-2.5 px-6 pb-6">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-[#E4E2DE] text-[0.8rem] text-[#706E69] hover:bg-[#F5F4F1] transition-all">
            Cancelar
          </button>
          <button onClick={() => { if (form.name.trim()) onSave(form) }}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl text-center">
        <div className="w-12 h-12 rounded-full bg-[rgba(246,90,112,0.1)] flex items-center justify-center mx-auto mb-4">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#F65A70" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/>
            <path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/>
          </svg>
        </div>
        ...
        <p className="text-[0.78rem] text-[#908E88] mb-5">
          ¿Seguro que quieres eliminar a <strong className="text-[#35322E]">{name}</strong> del equipo? Esta acción no se puede deshacer.
        </p>
...
function Toast({ message, onDone }: { message: string, onDone: () => void }) {
  return (
    <div className="fixed bottom-6 right-6 z-[60] flex items-center gap-3 bg-[#1A1815] text-white
      ...
      <span className="w-5 h-5 rounded-full flex items-center justify-center bg-[#9CEDC1] flex-shrink-0"
        style={{ boxShadow:'0 0 10px rgba(156,237,193,0.8)' }}>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#1A1815" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
...
export default function AdminEquipo() {
  ...
  const [team, setTeam]         = useState<TeamInfo>({
    ...
    description: 'Equipo multidisciplinario enfocado en producto digital, desarrollo frontend y experiencia de usuario. Iteración continua y entrega rápida.',
    avatarUrl: null,
  ...
  function handleSaveMember(m: Member) {
    ...
    showToast(editingMember?.id && editingMember.id !== '__new__' ? `${m.name} actualizado` : `${m.name} agregado al equipo`)
  }
  ...
  function openNewMember() {
    setEditingMember({ id:'__new__', initials:'', name:'', role:'Frontend Dev', email:'', status:'active',
      completionPct:0, tasksCompleted:0, tasksTotal:0,
      avatarGradient:AVATAR_GRADIENTS[0].gradient, avatarGlow:AVATAR_GRADIENTS[0].glow })
    setShowMemberModal(true)
  }

  function openEditMember(m: Member) { setEditingMember(m); setShowMemberModal(true) }
  ...
  return (
    <div className="relative min-h-screen bg-white font-body overflow-x-hidden">
      ...
      <header className="relative z-10 flex items-center justify-between px-4 sm:px-8 py-5 border-b border-[#EEECEA]">
        <div className="flex items-center gap-2.5 text-[1.05rem] font-semibold tracking-tight text-[#1A1815]">
          <span className="w-2.5 h-2.5 rounded-full flex-shrink-0"
            style={{ background:'linear-gradient(135deg,#60AFFF,#5961F9)', boxShadow:'0 0 14px rgba(96,175,255,0.85), 0 0 28px rgba(96,175,255,0.35)', animation:'pulseNeon 2.4s ease-in-out infinite' }}/>
          Orbit
        </div>
        <div className="flex items-center gap-3">
          <span className="hidden sm:flex items-center gap-1.5 text-[0.72rem] font-medium text-[#2e9660]">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background:'#9CEDC1', boxShadow:'0 0 10px rgba(156,237,193,1)', animation:'pulseNeon 1.8s ease-in-out infinite' }}/>
            En vivo
          </span>
      ...
      <main className="relative z-10 max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-9 pb-20">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 py-7">
          <div>
            ...
            <p className="text-[0.85rem] text-[#908E88] mt-1.5">Gestiona miembros, roles y atributos del equipo</p>
          </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              <p className="text-[0.68rem] font-semibold uppercase tracking-[0.1em] text-[#B0ADA7]">Miembros</p>
            ...
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              ...
              <div className="relative flex-1 sm:flex-none">
                <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-[#B0ADA7]" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round">
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-14 h-14 rounded-2xl bg-[#F5F4F1] flex items-center justify-center mb-4">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#B0ADA7" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/>
                  <path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
                </svg>
              </div>
              <p className="text-[0.85rem] font-medium text-[#706E69]">Sin resultados</p>
              <p className="text-[0.75rem] text-[#B0ADA7] mt-1">Ajusta los filtros o agrega un nuevo miembro</p>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
              ...
              <button onClick={openNewMember}
                ...
                <div className="w-9 h-9 rounded-xl bg-[#F5F4F1] group-hover:bg-[rgba(96,175,255,0.08)] flex items-center justify-center transition-colors">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                </div>
            ...
            <Panel>
              <div className="hidden sm:grid grid-cols-[36px_1fr_120px_80px_100px_64px] gap-3 px-4 py-2.5 border-b border-[#F5F4F1]">
                {['','Miembro','Rol','Estado','Progreso',''].map((h,i) => (
                  <span key={i} className="text-[0.65rem] font-semibold uppercase tracking-wider text-[#B0ADA7]">{h}</span>
                ))}
              </div>
              <div className="p-2">
                {filtered.map(m => (
                  <MemberListRow key={m.id} member={m} onEdit={openEditMember} onRemove={handleRemove}/>
                ))}
              </div>
            </Panel>
          )}
      ...
      {showMemberModal && editingMember && (
        <MemberModal member={editingMember} onSave={handleSaveMember} onClose={() => { setShowMemberModal(false); setEditingMember(null) }}/>
      )}
      {showTeamModal && (
        <TeamModal team={team} onSave={t => { setTeam(t); setShowTeamModal(false); showToast('Equipo actualizado') }} onClose={() => setShowTeamModal(false)}/>
      )}
      {deleteTarget && (
        <DeleteModal name={deleteTarget.name} onConfirm={confirmRemove} onClose={() => setDeleteTarget(null)}/>
      )}
