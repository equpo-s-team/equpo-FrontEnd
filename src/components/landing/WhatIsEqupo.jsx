...
export default function WhatIsEqupo() {
    ...
    return (
        <section id="what" className="relative py-24 px-[5vw] overflow-hidden" style={{ background: '#ffffff' }}>
            {/* Decorative blobs */}
            <div className="absolute top-0 right-0 w-[500px] h-[500px] rounded-full pointer-events-none"
                 style={{ background: 'radial-gradient(circle, rgba(96,175,255,0.10) 0%, transparent 70%)' }} />
            <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full pointer-events-none"
                 style={{ background: 'radial-gradient(circle, rgba(156,237,193,0.14) 0%, transparent 70%)' }} />
            ...
            <div className="max-w-[1160px] mx-auto grid md:grid-cols-2 gap-20 items-center relative z-10">
                    ...
                    <div className="relative">
                        ...
                        <div className="bg-white border border-grey-150 rounded-[24px] p-6 shadow-card-lg">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <KanbanCol
                                    ...
                                    tasks={[
                                        { tag: 'Diseño', tagBg: 'rgba(252,233,141,0.25)', tagColor: '#c48c00', text: 'Wireframes v2' },
                                        { tag: 'Dev', tagBg: 'rgba(156,237,193,0.25)', tagColor: '#1a7a4a', text: 'Setup API' },
                                    ]}
                                />
                                <KanbanCol
                                    title="En curso"
                                    ...
                                    tasks={[
                                        { tag: 'Collab', tagBg: 'rgba(134,240,253,0.25)', tagColor: '#1a7a8a', text: 'Revisión docs' },
                                        { tag: 'Dev', tagBg: 'rgba(156,237,193,0.25)', tagColor: '#1a7a4a', text: 'Auth module' },
                                    ]}
                                />
                                <KanbanCol
                                    title="Listo ✓"
                                    ...
                                    tasks={[
                                        { tag: 'Dev', tagBg: 'rgba(156,237,193,0.25)', tagColor: '#1a7a4a', text: 'DB schema' },
                                        { tag: 'Diseño', tagBg: 'rgba(252,233,141,0.25)', tagColor: '#c48c00', text: 'Style guide' },
                                    ]}
                                />
                            </div>
                        </div>
                        ...
                        <div className="absolute -bottom-6 -right-6 bg-white border border-grey-150 rounded-2xl px-5 py-4 shadow-card-lg flex items-center gap-3 min-w-[190px]">
                            <div className="w-11 h-11 rounded-xl flex-shrink-0"
                                 style={{ background: 'linear-gradient(135deg, #9CEDC1, #60AFFF)', animation: 'floatOrb 4s ease-in-out infinite' }} />
                            <div className="font-body text-[.75rem]">
                                <strong className="block font-semibold text-grey-800 text-[.82rem]">Mundo prosperando</strong>
                ...
                <div>
                    ...
                    <h2 className="font-maxwell text-display-lg text-grey-900 mb-5 reveal reveal-d1">
                        Un tablero colaborativo… con un giro que sí se nota.
                    </h2>

                    <p className="font-body text-[.97rem] leading-[1.75] text-grey-500 mb-5 reveal reveal-d2">
                        equpo es una plataforma para equipos de cualquier rubro que combina todo lo que necesitas para
                        trabajar bien y juntos.
                    </p>

                    <ul className="list-none flex flex-col gap-3 mb-7 reveal reveal-d2">
                        {[
                            { text: 'Organización clara — tableros, tareas, prioridades', grad: 'linear-gradient(135deg,#9CEDC1,#CEFB7C)' },
                            { text: 'Colaboración en tiempo real — chat, llamadas, miembros', grad: 'linear-gradient(135deg,#60AFFF,#86F0FD)' },
                            { text: 'Gamificación útil — puntos, niveles, registro de desempeño', grad: 'linear-gradient(135deg,#d99aee,#5961F9)' },
                            { text: 'Bienestar colectivo — un ambiente virtual que responde al trabajo del equipo', grad: 'linear-gradient(135deg,#FF94AE,#FCE98D)' },
                        ].map((item) => (
                            <li key={item.text} className="flex items-start gap-3 font-body text-[.93rem] text-grey-700">
                                <span
                    ...
                    <div
                        className="border-l-4 rounded-r-xl px-5 py-4 font-maxwell text-[1.05rem] text-grey-800 reveal reveal-d3"
                        style={{
...
export function SectionLabel({ children, gradient, textGrad, barColor, color, className = '' }) {
    ...
    const textStyle = textGrad ? { background: textGrad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' } : {};
