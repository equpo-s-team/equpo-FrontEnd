import type { OverdueTask, Priority } from '../types'
...
export function OverdueTable({ tasks }: OverdueTableProps) {
  return (
    ...
    >
      ...
      {/* Table header */}
      <div className="grid grid-cols-[1fr_auto_auto] gap-3 pb-2 border-b border-grey-150 relative z-10">
        <span className="text-[0.65rem] text-grey-400">Tarea</span>
        <span className="text-[0.65rem] text-grey-400">Días</span>
        <span className="text-[0.65rem] text-grey-400">Prio.</span>
      </div>

      {/* Scrollable rows */}
      <div
        className="max-h-[280px] overflow-y-auto relative z-10"
        ...
        {tasks.map(task => (
          ...
          >
            ...
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[0.67rem] font-semibold bg-[rgba(246,90,112,0.1)] text-[#c94155] whitespace-nowrap">
              +{task.daysOverdue} {task.daysOverdue === 1 ? 'día' : 'días'}
            </span>
            <span className={`inline-flex items-center justify-center px-2 py-0.5 rounded-full text-[0.65rem] font-semibold whitespace-nowrap ${PRIORITY_STYLES[task.priority]}`}>
              {task.priority}
            </span>
          </div>
        ))}
