...
export default function UserDropdown({ selected, onChange }) {
  ...
  const label =
    selected.length === 0
      ? 'Usuarios'
      : selected.length === USERS_DATA.length
        ? 'Todos'
      ...
      {open && (
        ...
        >
          ...
          <div className="px-3 pt-3 pb-2 border-b border-grey-100">
            <div className="flex items-center gap-2 bg-secondary rounded-[7px] px-2.5 py-2 border-[1.5px] border-transparent focus-within:border-blue focus-within:bg-blue/5 transition-all duration-200">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-medium text-grey-800 leading-tight">
                        {u.name}
                      </p>
          ...
          <div className="px-3 py-2 border-t border-grey-100 flex justify-between items-center">
            <button
              onClick={clearAll}
              className="text-[12px] text-grey-400 hover:text-red transition-colors font-body"
            >
              Limpiar todo
            </button>
            <button
              onClick={() => setOpen(false)}
              className="text-[12px] font-semibold text-blue bg-blue/8 border border-blue/25 px-3 py-1 rounded-lg hover:bg-blue/14 transition-colors font-body"
            >
              Aplicar
            </button>
          </div>
