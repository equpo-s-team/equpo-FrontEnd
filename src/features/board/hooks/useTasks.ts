...
export function useTasks(teamId: string, options: GetTeamTasksOptions = {}) {
  ...
  const transformSnapshot = useCallback((snapshot: Parameters<Parameters<typeof useFirestoreSubscription<{ tasks: TeamTask[]; meta: TaskListMeta }, FirestoreTaskDoc>>[2]>[0]) => {
