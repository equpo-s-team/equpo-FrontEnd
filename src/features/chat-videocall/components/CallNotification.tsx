...
export default function CallNotification() {
  ...
  const pendingCallsToNotify = activeCalls.filter(c => 
    !notifiedCalls.has(c.roomId) && 
    c.roomId !== activeVideoCall?.roomId
  );
  ...
  return (
    <div className="fixed top-20 right-8 z-[100] flex flex-col gap-3">
      {pendingCallsToNotify.map(call => {
        ...
        return (
          <div key={call.roomId} className="bg-white rounded-2xl shadow-2xl p-4 border border-grey-100 flex items-center gap-4 w-[320px] animate-in slide-in-from-right-8 duration-300">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-grey-500 truncate mt-0.5">Te invita a una videollamada en {roomName}</p>
            </div>
