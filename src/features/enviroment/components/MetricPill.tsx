...
export default function MetricPill({ icon, label, value, accent }: MetricPillProps) {
  return (
    ...
    >
      ...
      <span className="text-xs font-bold uppercase tracking-[0.5px] text-grey-700 whitespace-nowrap">
        {label}
      </span>
      <span className="text-sm font-bold text-grey-800 whitespace-nowrap">{value}</span>
    </div>
  );
}
