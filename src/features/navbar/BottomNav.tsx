      ...
      {navItems.map(({ id, label, icon: Icon, isActive }) => (
        <button
          ...
          className={`
            ...
            ${isActive ? 'text-blue-400' : 'text-gray-400 active:text-gray-300 dark:text-gray-400 dark:active:text-gray-300'}
          `}
        >
          {isActive && (
          ...
          <span
            className={`text-xs font-body font-medium leading-none transition-colors truncate max-w-full px-1 ${isActive ? 'text-blue-400' : ''}`}
          >
            {label}
          </span>
        </button>
        ...
        trigger={
          <button
            className={`
              flex-1 flex flex-col items-center justify-center gap-1
              py-3 relative transition-all duration-200
              ${isMoreActive ? 'text-blue-400' : 'text-gray-400 active:text-gray-300 dark:text-gray-400 dark:active:text-gray-300'}
            `}
          >
            {isMoreActive && (
              <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-blue-400" />
            ...
            <span
              className={`text-xs font-body font-medium leading-none ${isMoreActive ? 'text-blue-400' : ''}`}
            >
              Más
            </span>
          </button>
