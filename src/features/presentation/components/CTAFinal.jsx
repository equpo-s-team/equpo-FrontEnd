...
export default function CTAFinal() {
  return (
    <section
      id="cta-final"
      className="relative py-20 sm:py-28 px-4 sm:px-[5vw] overflow-hidden"
      style={{
        background: 'linear-gradient(150deg, #f0fdf6 0%, #eff8ff 45%, #fdf5ff 80%, #fff5f7 100%)',
      }}
    >
      <div
        className="hidden sm:block absolute -top-40 -right-40 w-[550px] h-[550px] rounded-full pointer-events-none"
        style={{ background: 'radial-gradient(circle, rgba(156,237,193,0.25), transparent 65%)' }}
      />
      <div
        className="hidden sm:block absolute -bottom-32 -left-32 w-[480px] h-[480px] rounded-full pointer-events-none"
      ...
      <div
        className="hidden sm:block absolute top-[10%] left-[5%] w-[180px] h-[180px] rounded-full border pointer-events-none"
        style={{ borderColor: 'rgba(156,237,193,0.3)' }}
      />
      <div
        className="hidden sm:block absolute bottom-[10%] right-[8%] w-[120px] h-[120px] rounded-full border pointer-events-none"
        style={{ borderColor: 'rgba(96,175,255,0.25)' }}
      />

      <div className="relative z-10 max-w-[820px] mx-auto text-center px-2">
        <SectionLabel
        ...
        <h2 className="font-maxwell text-2xl sm:text-3xl md:text-display-md text-grey-900 mb-3 sm:mb-4">
          Crea un equipo que se mueve en la misma dirección.
        </h2>

        <p className="font-body text-base sm:text-[1.05rem] leading-[1.75] text-grey-500 max-w-[560px] mx-auto mb-6 sm:mb-9">
        ...
        <a
          href="#"
          className="font-maxwell text-white text-base sm:text-[1.05rem] px-6 sm:px-10 py-3 sm:py-4 rounded-2xl no-underline inline-block mb-3 sm:mb-4 hover:-translate-y-1 transition-all"
          style={{
        ...
        <p className="font-body text-xs sm:text-[.82rem] text-grey-400 mb-8 sm:mb-10">
          "Empieza hoy. Ajusta detalles después. Tu equipo lo agradecerá."
        </p>

        <ul className="list-none flex justify-center flex-wrap gap-4 sm:gap-6">
          {[
            ...
            <li
              key={item.label}
              className="flex items-center gap-2 font-body text-xs sm:text-[.85rem] text-grey-600"
            >
              <span
