...
const DonutStatus: React.FC<DonutStatusProps> = ({ metrics }) => {
  ...
  // Orden de apilamiento: done, progress, qa, to-do, overdue
      ...
      <div className="donut-wrap">
        ...
        <div className="status-grid">
          <div className="status-card">
            <div
              className="s-dot"
              style={{ background: '#9CEDC1', boxShadow: '0 0 10px rgba(156,237,193,0.9)' }}
            ></div>
            <div>
              <div className="s-label">Done</div>
              <div className="s-val">
                {metrics.done} <span className="s-pct">({Math.round(percentages.done)}%)</span>
              </div>
            </div>
          </div>
          <div className="status-card">
            <div
              className="s-dot"
              style={{ background: '#86F0FD', boxShadow: '0 0 10px rgba(134,240,253,0.9)' }}
            ></div>
            <div>
              <div className="s-label">In Progress</div>
              <div className="s-val">
                {metrics.progress}{' '}
                <span className="s-pct">({Math.round(percentages.progress)}%)</span>
              </div>
            </div>
          </div>
          <div className="status-card">
            <div
              className="s-dot"
              style={{ background: '#9b7fe1', boxShadow: '0 0 10px rgba(155,127,225,0.8)' }}
            ></div>
            <div>
              <div className="s-label">To Do</div>
              <div className="s-val">
                {metrics.todo} <span className="s-pct">({Math.round(percentages.todo)}%)</span>
              </div>
            </div>
          </div>
          <div className="status-card">
            <div
              className="s-dot"
              style={{ background: '#FF94AE', boxShadow: '0 0 10px rgba(255,148,174,0.8)' }}
            ></div>
            <div>
              <div className="s-label">In QA</div>
              <div className="s-val">
                {metrics.qa} <span className="s-pct">({Math.round(percentages.qa)}%)</span>
              </div>
            </div>
          </div>
          <div className="status-card warn">
            ...
            <div style={{ flex: 1 }}>
              ...
              <div className="s-val" style={{ color: '#c94155' }}>
                {metrics.overdue}{' '}
                <span className="s-pct">({Math.round(percentages.overdue)}%)</span>
              </div>
            </div>
            <span style={{ fontSize: '.67rem', color: '#c94155', fontWeight: 500 }}>
              ⚠ atención
            </span>
          </div>
        </div>
