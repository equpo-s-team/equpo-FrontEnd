    ...
    return (
        <div className="panel neon-blue">
            <div className="panel-header">
                ...
                <span style={{fontSize: '.71rem', color: 'var(--grey-400)'}}>9 miembros · scroll para ver más</span>
            </div>
                ...
                {members.map(member => (
                    <div key={member.id} className="leg-item">
                        <div className="leg-dot"
                             style={{background: member.color, boxShadow: `0 0 7px ${member.color}`}}></div>
                        {member.name.split(' ')[0]}
                    </div>
                ))}
            </div>
                    ...
                    {members.map(member => {
                        ...
                        return (
                            <div key={member.id} className="member-row">
                                <div className={`avatar av-${member.initials.toLowerCase()}`}>{member.initials}</div>
                                <div>
                                ...
                                <div className="pct" style={{color: member.color}}>{percentage}%</div>
                            </div>
                        );
                    })}
                </div>
