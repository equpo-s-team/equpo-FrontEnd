...
export function OverdueTable({ tasks }: OverdueTableProps) {
  return (
    <div
      className="relative flex h-full min-h-0 flex-col bg-white dark:bg-gray-800 border border-grey-150 dark:border-gray-700 rounded-[14px] p-6 overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10 grid shrink-0 grid-cols-[1fr_auto_auto] gap-3 border-b border-grey-150 dark:border-gray-700 pb-2">
      ...
      >
        {tasks.map((task) => (
          <div
            key={task.id}
            className="grid grid-cols-[1fr_auto_auto] items-center gap-3 py-2.5 border-b border-grey-100 dark:border-gray-700 last:border-0 text-xs"
          >
            <div>
              <p className="font-medium text-grey-800 dark:text-gray-300 truncate">{task.task}</p>
              <p className="text-xs text-grey-500 dark:text-grey-400 mt-0.5">{task.assignee}</p>
            </div>
